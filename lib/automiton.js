...
function DFA(options){
	...
	this.run_with_animation = function(rwa){
		rwa = _.defaults(rwa, {speed:100,
			node:this.starting_node,
			strPos:0,
			onFinish: function(result){alert(result)}
		});
		if(rwa.node.options.onEnter) rwa.node.options.onEnter(rwa.string.charAt(rwa.strPos));
		setTimeout(function(){
			if(rwa.strPos >= rwa.string.length){
				return rwa.onFinish(rwa.node.is_accepting());	
			}
			
			var next = that.step(rwa.string.charAt(rwa.strPos), rwa.node);
			rwa.strPos++;
			if(!next){
				return rwa.onFinish(false);	
			} 
			if(rwa.node.options.onExit) rwa.node.options.onExit(rwa.string.charAt(rwa.strPos-1));
			rwa.node = next;
			that.run_with_animation(rwa);
		}, rwa.speed);
		console.log(rwa.speed);
	}
	...
	function Node(obj, id){
		...
		this.options = _.defaults(obj, {
			...
			onEnter: function(){console.log("entering: " + n.getId())},
			onExit: function(){console.log("exiting: " + n.getId())},
		});
