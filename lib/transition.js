...
function transition(e){
  ...
  if(nearest_state.geo.intersects(e.start)){
    ...
    if(nearest_end_state.geo.intersects(e.end)){
      prompt({
        title: "Enter Transition characters",
        placeholder: "Transition Characters",
        defaultText: nearest_state.node.getTransitionAsString(nearest_end_state.node),
        onOk: function(labelText){
          var start = nearest_state.getPosition();
          var end = nearest_end_state.getPosition();
          
          var transitions = labelText.split("");
          var transitionResult = null;
          var needToAddTransition = false;
          nearest_state.node.clearTransitionsToState(nearest_end_state.node);
          for(var i = 0; i < transitions.length; i++){

            transitionResult = nearest_state.node.add_transition(transitions[i], nearest_end_state.node);
          }
          var sid = nearest_state.getId();
          var eid = nearest_end_state.getId();
          // draw an arc between the two cirlces.
          if(typeof savedTransitions[sid + "-" + eid] !== 'undefined'){
            savedTransitions[sid + "-" + eid].clear();
            delete savedTransitions[sid + "-" + eid];
          ...
          t.text.setText(nearest_state.node.getTransitionAsString(nearest_end_state.node));
          tooltip_layer.draw();
        }
      });
    }
...
function transitionRep(newStartParent, newEndParent, lt){
  ...
  this.setParent = function(sp, ep){
    ...
    this.startParent = sp;
    this.endParent = ep
    sp.outgoingTransitions[ep.getId()] = that;
    ep.incomingTransitions[sp.getId()] = that;
  ...
  this.draw = function(){
    this.clear();
    var start = this.startParent.getPosition();
    var end = this.endParent.getPosition();
  ...
  this.clear = function(deleteReferences){
    ...
    if(deleteReferences){
      delete this.startParent.outgoingTransitions[this.endParent.getId()];
      delete this.endParent.incomingTransitions[this.startParent.getId()];
      delete savedTransitions[this.startParent.getId() + "-"+this.endParent.getId()];
    }
    if(this.tooltip){
      if(this.text){
        this.text.remove();
  ...
  this.draw(this.startParent.getPosition(), this.endParent.getPosition(), this.lt);
  return this;
}